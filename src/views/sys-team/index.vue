...
import { addTeamApi, getTeamTableApi, updateTeamApi } from '@/api/team/index'
import { type GetTeamTableResponseData, type IApiTeamInfoData } from '@/api/team/types/team'
    ...
    items: [
      {
        ...
        itemRender: {
          name: '$input',
          props: { placeholder: '请输入小组名称', clearable: true },
        },
      },
      {
        itemRender: {
    ...
    ajax: {
      query: ({ page, form }: VxeGridPropTypes.ProxyAjaxQueryParams) => {
        ...
        return new Promise<any>((resolve: Function) => {
          let total = 0
          let result: IApiTeamInfoData[] = []
          /** 加载数据 */
          const callback = (res: GetTeamTableResponseData) => {
            if (res && res.data) {
          ...
          /** 调用接口 */
          getTeamTableApi(params).then(callback).catch(callback)
        })
      },
    },
  },
  ...
  items: [
    {
      field: 'teamName',
      title: '小组名称',
      itemRender: { name: '$input', props: { placeholder: '请输入小组名' } },
    },
    {
      field: 'des',
      title: '小组职责',
      itemRender: { name: '$input', props: { placeholder: '请输入小组职责' } },
    },
    {
      field: 'leaderId',
      title: '小组主管',
...
const crudStore = reactive({
  ...
  /** 点击显示弹窗 */
  onShowModal: (row?: IApiTeamInfoData) => {
    if (row) {
      crudStore.isUpdate = true
      xModalOpt.title = '修改小组'
      // 赋值
      xFormOpt.data.id = row.id
      xFormOpt.data.teamName = row.teamName
      xFormOpt.data.des = row.des
      xFormOpt.data.leader = row.teamLeader
      xFormOpt.data.deptId = route.query.id
    }
    else {
      crudStore.isUpdate = false
      xModalOpt.title = '新增小组'
    }
  ...
  /** 确定并保存 */
  onSubmitForm: (row?: IApiTeamInfoData) => {
    if (xFormOpt.loading)
      return
    xFormDom.value?.validate((errMap?: VxeFormDefines.ValidateErrorMapParams) => {
      ...
      const callback = (err?: any) => {
        ...
        xFormOpt.data.leader = null
        !crudStore.isUpdate && crudStore.afterInsert()
        crudStore.commitQuery()
      }

      xFormOpt.data.teamLeaderId = xFormOpt.data.leader.id
      if (crudStore.isUpdate) {
        // 调用修改接口
        updateTeamApi(xFormOpt.data)
          .then(res => callback(res))
          .catch(err => callback(err))
      }
      else {
        // 调用新增接口
        addTeamApi(xFormOpt.data)
          .then(res => callback(res))
          .catch(err => callback(err))
      }
      xFormOpt.data = {}
  ...
  <div class="app-container">
    ...
    <vxe-grid ref="xGridDom" v-bind="xGridOpt">
      ...
      <template #toolbar-btns>
        <vxe-button v-if="route.query.id !== undefined" status="primary" icon="vxe-icon-add" @click="crudStore.onShowModal()">
          新增小组
        </vxe-button>
      </template>
      <!-- 操作 -->
      <template #row-operate="{ row }">
        ...
        <el-divider v-if="route.query.id !== undefined" direction="vertical" />
        <el-button v-if="route.query.id !== undefined" link type="warning" @click="crudStore.onShowModal(row)">
